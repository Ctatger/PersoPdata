...
def Graph_Clusters(X, unique_labels, colors, labels, n_clusters_):
    ...
    for k, col in zip(unique_labels, colors):
        ...
        if k == -1:
            ind = [index for index, element in enumerate(
                labels) if element == k]
...
def Cluster_Rectangles(X, unique_labels, labels, switch=False):
    ...
    for k in unique_labels:
        ...
        if switch:
            cluster_rectangles.append(
                (tuple(bottom_left[::-1]), tuple(top_right[::-1])))
        else:
...
if __name__ == "__main__":
    ...
    rec_colors = ["#%06x" % random.randint(
        0, 0xFFFFFF) for i in range(len(unique_labels))]
...
# %%
    ...
    rectangles_coordinates = Cluster_Rectangles(
        X, unique_labels, labels, switch=True)
    ...
    m = Map(layers=(map_layer, ), center=(
        (48.852, 2.246)), zoom=5, scroll_wheel_zoom=True)
